...
select distinct "Navn 4. Ledd", Beskrivelse, "OrgNr Bedrift",	"OrgNr Juridisk",	"OrgNrJuridiskEkstern", GLN from NPBP7_Enhet
select distinct "Navn 4. Ledd", Beskrivelse, "OrgNr Bedrift","OrgNr Juridisk","OrgNrJuridiskEkstern", GLN, Selskap from NPBP7_Enhet where Selskap ="True";

with c as (select distinct "JGZZ_FISCAL_CODE", Party_name, account_number , account_name, address1, city, postal_code, country from Customer where site_use_code = "SHIP_TO" ), 
e as (select distinct "Navn 4. Ledd", Beskrivelse, "OrgNr Bedrift",	"OrgNr Juridisk",	"OrgNrJuridiskEkstern", GLN from NPBP7_Enhet where Selskap ="True")
select c.*, e.* from c, e where c.JGZZ_FISCAL_CODE = e."OrgNr Juridisk" and c.JGZZ_FISCAL_CODE !="";


with c as (select distinct "Oraganization Number", "Vendor Name",	"Vendor Number", "ADDRESS_LINE1",	CITY,	ZIP,	COUNTRY from Supplier), 
e as (select distinct "Navn 4. Ledd", Beskrivelse, "OrgNr Bedrift",	"OrgNr Juridisk",	"OrgNrJuridiskEkstern", GLN from NPBP7_Enhet where Selskap ="True")
select c.*, e.* from c, e where c."Oraganization Number" = e."OrgNr Juridisk" and c."Oraganization Number" !="";



with c as (select distinct "JGZZ_FISCAL_CODE", Party_name, account_number , account_name, address1, city, postal_code, country from Customer where site_use_code = "SHIP_TO" ), 
e as (select distinct "OrgNr Juridisk"  from NPBP7_Enhet)
select c.*, e.* from c, e where c.JGZZ_FISCAL_CODE = e."OrgNr Juridisk" and c.JGZZ_FISCAL_CODE !="";
